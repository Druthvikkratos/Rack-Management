import { ContactUs, ILoginPaylod, IRole, IStaff, IclientName } from "../service/user.service"
import {
  Body,
  Controller,
  Delete,
...
import { ISaveData, UserService } from "../service/user.service"
...
export default class userControllers {
  ...
  public async loginUser(
    @Body() body: ILoginPaylod
  ): Promise<ILoginPaylod | null> {
    try {
  ...
  public async deleteStaff(@Param("user_fk") user_fk: string): Promise<string> {
    try {
      return await this.userService.deleteStaff(+user_fk)
    } catch (e) {
      throw new HttpException(
  ...
  @Get("/plan")
  public async getPlan(@Query("planId") planId: string): Promise<Plan[]> {
    try {
