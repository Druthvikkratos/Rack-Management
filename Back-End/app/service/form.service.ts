...
export class FormService {
  constructor(
    ...
    private itemFormsModel: typeof ItemForms,
  ...
  public async CreateForm(
    ...
    insert += ")"
    insert += "RETURNING id"
    const createItem = await sequelizeConfig.query(insert, {
  ...
  public async update(
    ...
    const num = await sequelizeConfig.query(
      `UPDATE ${schemaName}.${name}_template SET name = '${templateName}',description = '${payload.description
  ...
  public async deleteFromTemplate(
    ...
    const name = formname
    await sequelizeConfig.query(
      `DROP TABLE ${schemaName}.${name}_template`,
      {
        type: QueryTypes.DELETE
      }
    )
  ...
  public async findTemplates(name: string): Promise<Template> {
  ...
  public async findItemsOnTray(
    ...
    const formName = name
    const data = await sequelizeConfig.query(
      `  select c.*, t.quantity, t."upperLimit",  t."lowerLimit" ,t."notificationSettngFk" from "trayItems" t
      inner join ${schemaName}.${formName}_template c on c.id=t."formId"
      where t."tempId"=${itemTempId}  and t."trayId"= ${trayId}`,
      {
        type: QueryTypes.SELECT
      }
    )
  ...
  public async fetchUpperLowerAndNotification(
    ...
    const data = await sequelizeConfig.query(
      ` select ti."upperLimit", ti."lowerLimit", ti."notificationSettngFk" from "trayItems" ti where "formId"=${formId} and "trayId"=${trayId}`,
      {
        type: QueryTypes.SELECT
      }
    )
