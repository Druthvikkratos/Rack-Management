...
import {userAdmin,password, passwordMismatch} from './e2e-constants.cy'
...
describe('Login > Edit Profile', () => {
  it('EditUserProfileSuccessful', () => {
    ...
    cy.get('[id="Update Profile"]').click()
  ...
  it('EditUserProfileValidation', () => {
    ...
    cy.get('[ id="editUserProfileForm"]').contains('city is required')
...
describe('Login > Change Password', () => {
  it('ChangeUserPassword', () => {
    cy.login(userAdmin.username, userAdmin.password)
    cy.get('[class="user-profile dropdown"]').click('topRight')
    cy.get('[id="Change Password"]').contains("Change Password").click({multiple: true,force: true})
