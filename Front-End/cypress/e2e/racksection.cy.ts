...
import {userAdmin, updateStaffRecord, rackRecord, updateRackRecord} from './e2e-constants.cy'
...
describe('Company Admin Login > Rack > CRUD', () => {
  ...
  it('RackEdit', () => {
    ...
    // cy.get('[id="update_rack"]').contains('Update Rack').click({force: true})
    ...
    // cy.get('[id="select_stores"]').select(updateRackRecord.store)
    // cy.get('[id="update_rack"]').contains('Update Rack').click({force: true})
    // cy.visit('/racks')
    // cy.get('[id="racklisting"]')
  ...
  it('View Rack > Tray Crud', () => {
    ...
    // //Remove Tray removes tray but didnt updated in element so i used reolad here
    // cy.reload()
    ...
    // cy.get('[id="rack_name_change"]').should('contain.value', rackSelect.rackRename)
