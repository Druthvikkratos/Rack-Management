...
import { environment,userAdmin } from './e2e-constants.cy'
...
describe('Register > ComanyAdmin', () => {
  ...
  it('Activation', () => {   
    cy.task('connectDB',`SELECT * FROM users WHERE username='${userAdmin.username}'`).then( response => 
      {      
      console.log(response)  
      userData.clientFk =  response[0].clientFk 
      const id = response[0].id
      cy.request(`${environment.baseUrl}/api/user/activation/${userData.clientFk}/${id}`
       )
    })
  })
  it('Login', () => {
