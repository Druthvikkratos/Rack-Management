...
export class AppComponent {
      ...
      this.menuService
        ...
        .subscribe(data => {
          if (this.plan.name === 'Personal') {
            ...
            const updatedMenuList = this.menuService.templateDropdownRemoveUnderScore(data)
            this.logger.log(
  ...
  async fetchNotificationByUserFk(email: string): Promise<void> {
    ...
    this.userService.fetchNotificationByEmail(email).subscribe((response) => {   
      ...
      if (this.newnotifications != null && this.newnotifications != undefined && this.newnotifications.length > 0) {
  ...
  public async  updateNewNotificationToOld(): Promise<void> {
    const notificationIDs = this.newnotifications.map((notification) => notification.id)
    this.userService.updateNewNotificationToOld(notificationIDs).subscribe((data) =>
    {     
