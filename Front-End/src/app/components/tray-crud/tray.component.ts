...
import {
  KtdDragEnd, KtdDragStart, KtdGridComponent, KtdGridLayout, KtdGridLayoutItem, KtdResizeStart, ktdTrackById
...
export class TrayComponent implements OnInit, OnDestroy {
  ...
  async upload(): Promise<void> {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.uploadService.trayupload(this.currentFile,this.rackId,this.trayId).subscribe(
          (event: any) => {
            if (event.type === HttpEventType.UploadProgress) {
              ...
              this.file.filepath  + this.rackId  + "/" + this.trayId + "/" + this.file.filename
              if (this.fileId !== undefined) {
                ...
                this.file.filepath  + this.rackId  + "/" + this.trayId + "/" + this.file.filename
                this.uploadService.createTrayFile(this.file).subscribe(async () => {
                  this.alertService.success(
  ...
  removeTray(): void {
    ...
    this.rackService
      .deleteTrayById(this.currentlyBeingEditedTray.id)
      .subscribe(response => {
        this.trayObject = response
...
this.isInputOpen = true
  ...
  onClickBAck(trayList: Tray[]): void {
    this.changeTitle = false
    this.borderColor = ''
    this.menuService.isTrayMenu = false
    if(this.saveLayoutFlag=== true){
      this.showLayoutPopUp(trayList)
    }
  }

  showLayoutPopUp(trayList: Tray[]){
    swal({
      title:'layout Not Saved!',
      text: 'Layout is not saved, Do you want to save the layout?',
      type: 'warning',
