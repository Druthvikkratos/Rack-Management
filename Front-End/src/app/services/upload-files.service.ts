...
export class UploadFilesService {
  ...
  public trayupload(file: File,rackId:number,trayId:number): Observable<HttpEvent<FormData>> {
    const formData: FormData = new FormData()

    formData.append('image', file)

    const req = new HttpRequest('POST', `${this.baseUrl}/api/files/trayFile/${rackId}/${trayId}`, formData, {
      reportProgress: true,
      responseType: 'json'
    })
